...
import 'package:dess_explorer/domain/core/key_model.dart';
...
class SdkServices {
  ...
  Future<AtStatus> getAtSignStatus(String atSign) async => AtStatusImpl(
        rootUrl: AtConstants.rootDomain,
      ).get(atSign);
    ...
    try {
      ...
      for (final key in list) {
        if (key.key == 'profilepic' &&
            key.namespace == AtConstants.appNamespace) {
  ...
  Future<String?> getUserName() async {
    ...
    try {
      final _names =
          await getAllKeys(regex: 'name.${AtConstants.appNamespace}');
